...
- Optional live streaming via Kite Connect
...
def threshold_search(
    ...
    out["combined_score"] = out["total_net_return"] * 0.60 + out.get("recall_pos", pd.Series(0.0, index=out.index)) * 0.40
...
def trade_permitted(
    ...
    if signal == "BUY" and prob_up < risk.min_confidence:
...
if start_feed_btn:
    if mode != "LIVE":
        ...
        try:
            ...
            token = st.session_state.kite_broker.lookup_instrument_token(market, tradingsymbol)
...
if mode == "LIVE" and st.session_state.live_builder is not None:
...
if trained_model is not None and feature_cols is not None and not live_feature_frame.empty:
    for c in feature_cols:
        if c not in live_feature_frame.columns:
            live_feature_frame[c] = 0.0
    live_feature_frame = live_feature_frame.sort_index()
...
if plot_source is not None and not plot_source.empty:
    ...
    fig.add_trace(go.Scatter(x=plot_source.index, y=plot_source["Close"].rolling(20).mean(), mode="lines", name="MA 20"))
